...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point supporting ``python -m design.bedrock <subcommand>``.

    Mirrors the argument contract in :mod:`design.__main__` so one definition
    of truth exists for the CLI surface. ``__main__`` reuses this via the
    ``python -m design`` invocation.
    """
    import argparse
